...
namespace DMI.Models
{
    ...
    public class WeatherDataProvider
    {
        ...
        public void GetPollenData(Action<IEnumerable<PollenItem>, Exception> callback)
        {
            ...
            client.DownloadStringCompleted += (sender, e) =>
            {
                if (e.Error != null)
                {
                    callback(default(IEnumerable<PollenItem>), e.Error);
                }
        ...
        /// </summary>
        /// <param name="callback"></param>
        public void GetNewsItems(Action<IEnumerable<NewsItem>, Exception> callback)
        {
            var client = new WebClient()
            {
                Encoding = Encoding.GetEncoding("iso-8859-1")
            ...
            client.DownloadStringCompleted += (sender, e) =>
            {
                if (e.Error != null)
                {
                    callback(default(IEnumerable<NewsItem>), e.Error);
                }
